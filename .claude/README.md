...
Scope rule: this harness contains **only** what serves building the Forge **Design
...
├── skills/              # discoverable, Forge-native skills (auto-trigger)
...
(`FORGE-DS-REFERENCE.md`, `llms.txt`), then composes with the Forge classes/components in
`src/styles/ds.css` and `src/ds/`. There is no daemon — the agent reads these files; the
anti-AI-slop P0 list is a mandatory manual checklist. Forge artifacts are built **in the
repo** (the 3-file DS-page ritual), not emitted as standalone artifacts.

## Distribution

`npm run build:plugin` packages this directory into `plugin/forge-harness/` (with portable
