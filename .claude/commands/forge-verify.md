---
name: forge-verify
description: Run the Forge component contract gate (npm run forge:verify) for one component or the whole catalog, and report clause-by-clause Done/Blocked status across the four surfaces.
argument-hint: "[<slug> | --all] [--heavy]"
allowed-tools: [Bash, Read]
---

# /forge-verify — the contract gate

The bar is `packages/registry/forge.contract.json`. A component is **Done** iff
`npm run forge:verify -- --component <slug> --strict` exits 0. The verifier's green beats
any opinion.
...
2. Run the gate for the requested scope:
   - one component: `npm run forge:verify -- --component $ARGUMENTS --strict`
   - the catalog: `npm run forge:verify -- --all` (report) — add `--strict` to gate.
