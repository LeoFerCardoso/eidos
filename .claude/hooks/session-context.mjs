...
- LAYOUT & ALIGNMENT (.claude/craft/layout-and-spacing.md — mandatory checklist; alignment is a PIXEL property, verify on a screenshot not in the CSS): one column / one left edge — header (eyebrow, title, support line) and body (tabs, toolbar, every list row's surface) share the same inline-start edge. Spacing lives INSIDE items as padding, NEVER as margin-inline on a list/table row (an outer margin breaks column alignment). A list/table row is SQUARE (no border-radius) with a hairline divider, not a rounded floating chip. FLAT-FIRST: a card only separates 2+ distinct concerns; one subject = no card; never nest cards; watch carded-by-default components (TabsContent/.eidos-tabpanel ships padding+border+bg — flatten it for a single list). Before "done", screenshot and drop a vertical guide on the title's left edge: header + tabs + toolbar + rows must all meet it.
