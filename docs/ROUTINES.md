# Eidos harness — scheduled routines & loops
...
> Clone the eidos repo. Read `.claude/skills/component-page/references/checklist.md`
...
> Clone eidos. Check `package.json` against the latest `ai`, `@ai-sdk/react`, `next`,
...
> A PR was opened on eidos. Use the `code-reviewer` subagent and the `eidos-reviewer`
> output style. Review the diff against the Eidos invariants, the DS discipline rules, the
...
> Clone eidos, `npm ci`, `npm run build`. If the build fails, open an issue with the
