...
//   • optional strength bar (.pw-strength) reflecting how many requirements pass
//   • optional requirements checklist (.pw-reqs) with check/x icons per item
//
// Strength scoring: 0 (blank) → 4 tiers (weak/fair/strong/very-strong),
// mapped from the number of passing requirements. The tones are:
//   1/4  → danger   (#F87171)
//   2/4  → warning  (#FBBF24)
//   3/4  → success  (#34D399)
//   4/4  → success  (#34D399, higher opacity)
//
// All CSS lives in tokens.css (.pw-*). No per-component <style> blocks.
// aria-live="polite" on the strength label; the requirements list is a real
// <ul role="list"> so VoiceOver/NVDA announces each item.
...
// Sensible defaults — a secure-enough baseline for most applications.
...
// ── PasswordField ─────────────────────────────────────────────────────────────
// Composite: PasswordInput with showStrength=true and DEFAULT requirements.
// Convenience export for the common "new password" form use-case.
export interface PasswordFieldProps extends Omit<PasswordInputProps, 'showStrength'> {
  /** Override the default set of requirements. */
  requirements?: PasswordRequirement[];
}

export function PasswordField({ requirements = DEFAULT_PASSWORD_REQUIREMENTS, ...rest }: PasswordFieldProps) {
