...
// (N stacked bars, last one shorter).
//
// CSS lives in packages/ui/styles/ds.css (.sk / .sk-shimmer block).
// No <style> here — className strings only.
//
// A11y: skeletons are decorative; each shape is aria-hidden="true". The
// optional `label` prop emits an sr-only "Loading…" span inside a
// role="status" wrapper on the outermost element (multi-line mode). Single
// shapes stay bare spans (the caller's container should carry aria-busy).
//
// prefers-reduced-motion: the shimmer sweep stops and the shape rests as a
// flat tinted block — enforced by a @media guard in ds.css.

// ── Types ────────────────────────────────────────────────────────────────────

export type SkeletonVariant = 'line' | 'box' | 'circle';
...
// ── Resolve inline styles for a single shape ──────────────────────────────────
...
/** General-purpose loading placeholder. Composes line/box/circle shapes with
 *  optional multi-line mode and a built-in screen-reader announcement. */
