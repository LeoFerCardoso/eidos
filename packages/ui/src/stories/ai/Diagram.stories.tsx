...
const meta = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component:
          'A styled figure surface for rendered diagrams (e.g. Mermaid, Graphviz, custom SVG) ' +
          'inside a model reply. The DS does not bundle a renderer — pass the rendered ' +
          '<svg> or markup as children and supply an optional figcaption via `caption`. ' +
          'Use when the model produces a flow, sequence, or architecture diagram that ' +
          'should be visually separated from prose.',
      },
    },
  },
  argTypes: {
...
/** Default — a service data-flow diagram with a caption. */
...
/** No caption — bare diagram without an annotation. */
...
/**
 * Variants — flow diagram types side by side: sequential pipeline, branching
 * incident triage, and a two-tier service dependency.
 */
  ...
  render: () => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 32 }}>
      ...
      <Diagram caption="Canary rollout pipeline — Ring 0 → Ring 1 → Ring 2 → GA">
        <svg viewBox="0 0 480 80" width="480" height="80" style={{ display: 'block' }} aria-label="Canary rollout stages">
          <defs>
            <marker id="arr-v1" markerWidth="7" markerHeight="7" refX="7" refY="3.5" orient="auto">
              <path d="M0,0 L7,3.5 L0,7 Z" fill="var(--fg-muted)" />
            </marker>
          </defs>
          {['Ring 0', 'Ring 1', 'Ring 2', 'GA'].map((label, i) => (
            <React.Fragment key={label}>
              ...
              {i < 3 && (
                <line x1={104 + i * 118} y1="40" x2={118 + i * 118} y2="40" stroke="var(--fg-muted)" strokeWidth="1.5" markerEnd="url(#arr-v1)" />
              )}
            </React.Fragment>
          ))}
        </svg>
      ...
      <Diagram caption="Incident triage decision — alert classification branch">
        <svg viewBox="0 0 360 130" width="360" height="130" style={{ display: 'block' }} aria-label="Incident triage branch">
          <defs>
      ...
      <Diagram caption="pix-router dependency graph — direct runtime dependencies">
        <svg viewBox="0 0 400 110" width="400" height="110" style={{ display: 'block' }} aria-label="pix-router service dependency graph">
          <defs>
