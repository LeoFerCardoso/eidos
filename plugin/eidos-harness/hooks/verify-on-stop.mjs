...
function main() {
  const contractPath = resolve(ROOT, 'packages/registry/eidos.contract.json');
  ...
  if (failing.length) {
    process.stderr.write(
      ...
      `\nFix these (or add a reviewed waiver in eidos.contract.json) and re-verify with \`npm run eidos:verify -- --component <slug> --strict\`.\n`,
    );
    process.exit(2);
  }
  process.exit(0);
