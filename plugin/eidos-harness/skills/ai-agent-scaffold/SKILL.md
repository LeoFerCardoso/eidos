...
description: Use when adding a tool-using AI agent to the eidos app — generates a type-safe ToolLoopAgent (Zod tools, stopWhen, exported InferAgentUIMessage type) under lib/ai/agents/ plus the matching streaming route and useChat client wiring. Produces the agent module, not just docs.
