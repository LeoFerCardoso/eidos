...
description: Use when configuring or standardizing model selection for AI features in the eidos app — switches the repo to AI Gateway 'provider/model' strings (no provider SDK imports), with ordered fallbacks, cost caps, and per-feature usage tags, authed via VERCEL_AI_GATEWAY_API_KEY. Use whenever a model is being chosen or hardcoded.
