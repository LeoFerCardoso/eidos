...
name: skill-template
description: Template for authoring a new Forge harness skill. Invoke with /skill-template when creating a new ${CLAUDE_PLUGIN_ROOT}/skills/<name>/SKILL.md. Quiet (not auto-triggered).
...
> Forge-native: they build **in the repo**, never emit standalone `<artifact>` HTML.
...
Forge classes, never write per-page CSS").
...
## Hard rules (the Forge invariants every skill enforces)
