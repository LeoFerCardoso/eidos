---
name: ai-chat-ui
description: Use when building the client chat interface for an AI feature in the forge-ds app — generates a 'use client' useChat component with DefaultChatTransport, manual useState input, message.parts rendering, and state-gated typed tool parts, styled with Forge ds.css / ai-shell.css classes.
...
## Forge styling — use existing classes, never inline `<style>`
...
- For polished message/prompt primitives, install **AI Elements** via the shadcn registry
  (invoke the `vercel:shadcn` skill) but keep Forge classes for layout.
