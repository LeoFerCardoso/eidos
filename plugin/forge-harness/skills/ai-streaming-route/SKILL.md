...
description: Use when creating the server endpoint for an AI chat/agent feature in the forge-ds app — generates a Next.js App Router Route Handler that calls streamText/agent and returns toUIMessageStreamResponse for a useChat client, with stop support and an optional Redis-backed resumable-streams variant.
...
export async function POST(req: Request) {
  ...
  const result = streamText({
    ...
    system: 'You are the Forge assistant. Be precise and concise.',
