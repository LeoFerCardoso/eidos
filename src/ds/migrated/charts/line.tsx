...
export function Demo({ data }) {
  ...
  return (
    <ForgeChart title="Requests / minute" meta="last 24h" height={280}>
      <LineChart data={data}>
    ...
    return (
      <Section id="chart-line" title="Line chart" desc="Connects ordered points to show how a continuous value moves over time — slope is the message. Reach for it when trend and rate of change matter most: RPS, latency percentiles, queue depth, SLO compliance.">
        ...
        <Frame label="single series · requests-per-minute · 24h" code={USAGE}>
          <ForgeChart title="Requests / minute" subtitle="forge-api" meta="last 24h" height={260}>
        ...
        <Frame label="multi-series · p50 / p95 / p99 latency">
          <ForgeChart title="Latency percentiles" subtitle="forge-api" meta="ms" height={260}>
            <LineChart data={LAT} margin={{ top: 8, right: 12, left: -8, bottom: 0 }}>
              <CartesianGrid {...forgeGridProps}/>
              <XAxis dataKey="t" {...forgeXAxisProps} interval={4}/>
              <YAxis {...forgeYAxisProps}/>
              <Tooltip content={<ForgeTooltipContent/>}/>
        ...
        <Frame label="reference line · SLO target at 99%">
          <ForgeChart title="Build success rate" subtitle="ci-runner" meta="14 days" height={260}>
            <LineChart data={RATE} margin={{ top: 8, right: 12, left: -8, bottom: 0 }}>
              <CartesianGrid {...forgeGridProps}/>
              <XAxis dataKey="day" {...forgeXAxisProps}/>
              <YAxis {...forgeYAxisProps} domain={[96, 100]}/>
              <Tooltip content={<ForgeTooltipContent/>}/>
        ...
        <Frame label="dashed · forecast or projection">
          <ForgeChart title="Active workflows" subtitle="forecast" meta="next 14d" height={220}>
          ...
          <div className="surface" style={{padding: 18}}>
            ...
            <details className="surface" style={{marginBlockStart: 12, padding: '10px 12px'}}>
              ...
              <table className="spec" style={{marginBlockStart: 10}}>
                <caption style={{captionSide: 'top', textAlign: 'start', color: 'var(--fg-faint)', fontSize: 'var(--text-xs)', marginBlockEnd: 8}}>forge-api · requests per minute, last 24h (every 3rd hour)</caption>
                <thead>
                  <tr>
        ...
        <Frame label='dir="rtl" — title, legend and tooltip align right; time still flows left to right'>
          <div dir="rtl" style={{width: '100%'}}>
            <ForgeChart title="الطلبات في الدقيقة" subtitle="forge-api" meta="آخر ٢٤ ساعة" height={240}>
              <LineChart data={RPS} margin={{ top: 8, right: 12, left: -8, bottom: 0 }}>
                <CartesianGrid {...forgeGridProps}/>
                <XAxis dataKey="hour" {...forgeXAxisProps} interval={3}/>
                <YAxis {...forgeYAxisProps}/>
            ...
            <div className="body">
              <ForgeChart height={140} padding={8}>
                <LineChart data={RPS} margin={{ top: 4, right: 8, left: -16, bottom: 0 }}>
                  <CartesianGrid {...forgeGridProps}/>
                  <XAxis dataKey="hour" {...forgeXAxisProps} interval={5}/>
                  <YAxis {...forgeYAxisProps}/>
        ...
        <PropsTable
          label="<ForgeChart /> + Recharts <Line />"
          rows={[
