...
import {
  ...
  type SkillDef, type ContextDef, type AppDef, type ChannelConsumer,
...
const STEPS = [
  ...
  { id: 'channels',  title: 'Channels',   sub: 'Where it runs' },
  { id: 'safety',    title: 'Safety',     sub: 'Guardrails & policy' },
  { id: 'review',    title: 'Review',     sub: 'Confirm & create' },
] as const;

// Channel groups for the wizard step (ordered; consumer-keyed).
const CHANNEL_GROUPS: { consumer: ChannelConsumer; label: string; hint: string }[] = [
  { consumer: 'Humans',    label: 'In Forge',        hint: 'Where people use the agent inside the platform.' },
  { consumer: 'Messaging', label: 'Messaging apps',  hint: 'Let people reach the agent on the apps they already use. Each needs a one-time connection and a security review.' },
  { consumer: 'Systems',   label: 'Programmatic',    hint: 'Call the agent from your own systems.' },
  { consumer: 'Agents',    label: 'Agent to agent',  hint: 'Let other agents discover and call this one.' },
];
...
export default function NewAgent({ editId }: { editId?: string } = {}) {
  ...
  const step0Ok = nameOk && instrOk && summaryOk;
  ...
  const StepChannels = (
    <>
      <StepHead title="Channels" desc="Choose where this agent is available. Forge chat is always on; messaging and programmatic surfaces each need a one-time connection and a security review before they go live." />
      {CHANNEL_GROUPS.map((g) => {
        const items = CHANNELS.filter((c) => c.consumer === g.consumer);
        if (items.length === 0) return null;
        return (
          <Field key={g.consumer} label={g.label} hint={g.hint}>
            <div className="fp-wizard-rvchannels is-setup">
              {items.map((c) => {
  ...
  const StepReview = (
    <>
      ...
      <div className="fp-wizard-review">
        ...
        <div className="rv-block">
          <span className="rv-h">Channels <span className="rv-count">{CHANNELS.filter((c) => channels.has(c.id)).length}</span></span>
  ...
  if (isEdit) {
    const EDIT_SECTIONS = [StepDefine, StepModel, StepKnowledge, StepApps, StepChannels, StepSafety];
    return (
  ...
  const BODY = [StepDefine, StepModel, StepKnowledge, StepApps, StepChannels, StepSafety, StepReview][step];
